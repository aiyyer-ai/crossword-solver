...
//colors
let crosswordBlack = 0x000000;
let crosswordWhiteHex = `ffffff`;
let crosswordWhite = parseInt(`0x${crosswordWhiteHex}`);
let crosswordText = 0x333333;
let crosswordScrollBar = 0xe5e5e5;
let crosswordScrollButton = 0x7e7e7e;
let crosswordWrongRed = 0xff4d4d;
let crosswordHighlightPrimary = 0xfae522;
let crosswordHighlightSecondary = 0xbfe5ff;
let crosswordCorrect = 0x005c99;

//Darkmodecolors
// let crosswordBackground = 0x171718;
// let crosswordBlack = 0x171718;
// let crosswordWhiteHex = `171718`;
// let crosswordWhite = 0x585962;
// let crosswordText = 0xf5f6f8;
// let crosswordScrollBar = 0xBBBBBB;
// let crosswordScrollButton = 0x7e7e7e;
// let crosswordWrongRed = 0xff4d4d;
// let crosswordHighlightPrimary = 0x5478a5;
// let crosswordHighlightSecondary = 0x463e7b;
// let crosswordCorrect = 0x5478a5;
...
function createBoard(info) {
	...
	render.backgroundColor = crosswordBlack;
  ...
  document.getElementById("html").style.backgroundColor = `#${crosswordWhiteHex}`;
	...
	titleFiller.beginFill(crosswordBackground);
	titleFiller.drawRect(0, 0, (boardWidth * 36) + (clueWidth * 2) + 2, 50);
	titleFiller.zIndex = 0;
	titleContainer.addChild(titleFiller);

	let titleText = new PIXI.Text(` ${info.title} `,{fontFamily: squareFont, fontSize: 24, fill : crosswordText, align : 'left',  fontWeight : 'bold' });
	let authorText = new PIXI.Text(`by ${info.author}`,{fontFamily: squareFont, fontSize: 18, fill : crosswordText, align : 'left'});
	let timerText = new PIXI.Text(`0:00`,{fontFamily: squareFont, fontSize: 18, fill : crosswordText, align : 'left'});
...
//darkmode button
	// let darkContainer = new PIXI.Container();
	// titleContainer.addChild(darkContainer);
	// let darkButton = new PIXI.Graphics();
	// darkButton.beginFill(0xffffff);
	// darkButton.lineStyle(2, crosswordScrollBar, 1);
	// darkButton.drawRoundedRect(0, 0, 80, 40, 5);
	// darkButton.interactive = true;
	// let darkbuttonRect = darkButton.getLocalBounds();
	// darkContainer.addChild(darkButton);
	// darkButton.tint = crosswordWhite;
	// darkContainer.y = (titleFiller.height)/2 - (darkbuttonRect.height)/2;
	// darkContainer.x = ((boardWidth * 36) + (clueWidth * 2) + 2) - 80 - 90;
	// let darkButtonIconDayTexture = PIXI.Texture.from('assets/sun.png');
	// let darkButtonIconNightTexture = PIXI.Texture.from('assets/moon.png');
	// let darkButtonIconDay = new PIXI.Sprite(darkButtonIconDayTexture);
	// let darkButtonIconNight = new PIXI.Sprite(darkButtonIconNightTexture);
	// darkButtonIconNight.anchor.set(0.5, 0.5);
	// darkButtonIconDay.anchor.set(0.5, 0.5);
	// darkButtonIconDay.height = 40;
	// darkButtonIconNight.height = 40;
	// darkButtonIconDay.width = 40;
	// darkButtonIconNight.width = 40;
	// darkButtonIconNight.visible = false;
	// darkContainer.addChild(darkButtonIconDay);
	// darkContainer.addChild(darkButtonIconNight);
	// darkButtonIconDay.y = darkButton.height/2;
	// darkButtonIconDay.x = darkButton.width/2;
	// darkButtonIconNight.y = darkButton.height/2;
	// darkButtonIconNight.x = darkButton.width/2;
	// darkButton.on('pointerover', (event) => onDarkOver(darkButton, darkButtonIconNight, darkButtonIconDay));
	// darkButton.on('pointerout', (event) => offDarkOver(darkButton, darkButtonIconNight, darkButtonIconDay));
	// darkButton.on('pointerdown', (event) => onDarkClick(darkButton, event, info));
	// darkButton.on('pointerup', (event) => offDarkClick(darkButton));
	...
	let acrossText = new PIXI.Text(` ACROSS `,{fontFamily: squareFont, fontSize: 18, fill : crosswordText, align : 'left',  fontWeight : 'bold' });
	...
	acrossBacking.tint = crosswordBackground;
	let acrossLine = new PIXI.Graphics();
	acrossLine.beginFill(crosswordScrollBar);
	...
	for (const [index, acrossClue] of Object.entries(info.clues.Across)) {
		const clueNum = new PIXI.Text(` ${String(acrossClue[0])}  `,{fontFamily: squareFont, fontSize: 18, fill : crosswordText, align : 'left',  fontWeight : 'bold' });
		let clueNumRect = clueNum.getLocalBounds();
		const clue = new PIXI.Text(`${String(acrossClue[1])}`,{fontFamily: squareFont, fontSize: 18, fill : crosswordText, align : 'left', wordWrap : true, wordWrapWidth: (245 - clueNumRect.width)});
	...
	scrollbarAcross.beginFill(crosswordScrollBar);
	scrollbarAcross.drawRect(0, 0, scrollbarWidth, (boardHeight * 36) + 2);
	scrollbarAcross.interactive = true;
	scrollbarAcross.hitArea = new PIXI.Rectangle(0, 0, scrollbarWidth, (boardHeight * 36) + 2);
	...
	scrollbuttonAcross.beginFill(crosswordScrollButton);
	scrollbuttonAcross.drawRect(0, 0, scrollbarWidth, scrollbuttonSizeAcross);
	scrollbuttonAcross.interactive = true;
	scrollbuttonAcross.hitArea = new PIXI.Rectangle(0, 0, scrollbarWidth, scrollbuttonSizeAcross);
	...
	let downText = new PIXI.Text(` DOWN `,{fontFamily: squareFont, fontSize: 18, fill : crosswordText, align : 'left',  fontWeight : 'bold' });
	...
	downBacking.tint = crosswordBackground;
	let downLine = new PIXI.Graphics();
	downLine.beginFill(crosswordScrollBar);
	...
	for (const [index, downClue] of Object.entries(info.clues.Down)) {
		const clueNum = new PIXI.Text(` ${String(downClue[0])}  `,{fontFamily: squareFont, fontSize: 18, fill : crosswordText, align : 'left',  fontWeight : 'bold' });
		let clueNumRect = clueNum.getLocalBounds();
		const clue = new PIXI.Text(`${String(downClue[1])}`,{fontFamily: squareFont, fontSize: 18, fill : crosswordText, align : 'left', wordWrap : true, wordWrapWidth: (245 - clueNumRect.width)});
		let clueRect = clue.getLocalBounds();
		let clueContainer = new PIXI.Container();
		let clueSpotHeight = clueRect.height + 16;
	...
	scrollbarDown.beginFill(crosswordScrollBar);
	scrollbarDown.drawRect(0, 0, scrollbarWidth, (boardHeight * 36) + 2);
	scrollbarDown.interactive = true;
	scrollbarDown.hitArea = new PIXI.Rectangle(0, 0, scrollbarWidth, (boardHeight * 36) + 2);
	...
	scrollbuttonDown.beginFill(crosswordScrollButton);
	scrollbuttonDown.drawRect(0, 0, scrollbarWidth, scrollbuttonSizeDown);
	scrollbuttonDown.interactive = true;
	scrollbuttonDown.hitArea = new PIXI.Rectangle(0, 0, scrollbarWidth, scrollbuttonSizeDown);
	...
	for (let row in info.puzzle) {
		...
		for (let [index, square] of Object.entries(info.puzzle[row])) {
			...
			if (square.style) {
				crosswordSquare.lineStyle(1, crosswordBlack, 1);
				crosswordSquare.drawCircle(squareSize/2, squareSize/2, (squareSize/2));
				square = square.cell;
			}
			crosswordSquare.interactive = true;
			crosswordSquare.hitArea = new PIXI.Rectangle(0, 0, squareSize, squareSize);
			...
			crosswordSquare.tint = crosswordWhite;
			squarePosition++;
			squareContainer.addChild(crosswordSquare);
			if(square != 0 && typeof square == 'number') {
				const text = new PIXI.Text(String(square),{fontFamily: squareFont, fontSize: 12, fill : crosswordText, align : 'left'});
				clueNumber = square;
				text.x = 1;
				text.y = -1;
			...
			wrong.lineStyle(2, crosswordWrongRed, 1);
	...
	if(currentHighlight.object) {
		...
		if (key.length == 1) {
			if(checkedCorrect.indexOf(clickedPos.toString()) == -1) {
				...
				const letter = new PIXI.Text(key.toUpperCase(),{fontFamily : squareFont, fontSize: 26, fill : crosswordText, align : 'left'});
...
function removeOldText(info, del, recursion) {
	...
	if(currentHighlight.object.children[0] ? (currentHighlight.object.children[currentHighlight.object.children.length - 1].name == 'guess') : currentHighlight.object.children[0])  {
		...
		clueAcross.children[0].children[0].style.fill = crosswordText;
		clueAcross.children[0].children[1].style.fill = crosswordText;
		clueDown.children[0].children[0].style.fill = crosswordText;
		clueDown.children[0].children[1].style.fill = crosswordText;
	} else if (del) {
		if(!recursion) {
...
function setHighlight(clickee, adjust = false) {
	...
	if(currentHighlight.object) {
		currentHighlight.object.tint = crosswordWhite;
		for (const blueLight of currentHighlight.otherSquares) {
			blueLight.tint = crosswordWhite;
		}
	}
	currentHighlight.object = clickee;
	clickee.tint = crosswordHighlightPrimary;
	...
	let clueDown = downClueContainer.getChildByName(clickee.parent.clues.down);
		...
		if(currentHighlight.across) {
			clueAcross.children[0].tint = crosswordHighlightSecondary;	
			clueDown.children[0].tint = crosswordScrollButton;		
		} else {
			clueAcross.children[0].tint = crosswordScrollButton;	
			clueDown.children[0].tint = crosswordHighlightSecondary;				
		}
	...
	//clickedPos[0] = x // clickedPos[1] = y //
		...
		while(search) {
			...
			if (leftSquare && !leftDone) {
				...
				leftData.tint = crosswordHighlightSecondary;
			} else {
				leftDone = true;
			}
			if (rightSquare && !rightDone) {
				...
				rightData.tint = crosswordHighlightSecondary;
			} else {
				rightDone = true;
			}
...
function onButtonClick(button, event, info) {
	button.tint = crosswordHighlightSecondary;
	checkAnswers(info);
}

function offButtonClick(button, event, info) {
	button.tint = crosswordWhite;
}
...
function checkAnswers(info) {
	...
	for(const correctAnswerPosition of correctAnswers) {
		...
		correctAnswer.children[0].children[correctAnswer.children[0].children.length - 1].style.fill = crosswordCorrect;
...
function onDarkOver(button, open, closed) {
	open.visible = true;
	closed.visible = false;
}

function offDarkOver(button, open, closed) {
	closed.visible = true;
	open.visible = false;
}

function onDarkClick(button, event, info) {
	button.tint = crosswordHighlightSecondary;
}

function offDarkClick(button, event, info) {
	button.tint = crosswordWhite;
}
