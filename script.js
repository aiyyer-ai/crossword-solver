...
function createBoard(info) {
	...
	app = new PIXI.Application({ width: (boardWidth * 36) + 2 + 401, height: (boardHeight * 36) + 2, resolution: 4, antialias: true })
	let render = app.renderer;
	render.backgroundColor = 0x000000;
    render.view.style.position = "absolute";
    render.view.style.display = "block";
    render.view.style.width = `${(boardWidth * 36) + 2 + 401}px`;
	...
	clueArea.drawRect(0, 0, 400, (boardHeight * 36));
